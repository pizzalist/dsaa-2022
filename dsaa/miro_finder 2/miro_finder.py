...
def get_maze_answer(maze : dict, end_point=(1,1)) -> list:
    ...
    while len(stack)>0:
        ...
        if maze[(X,Y)] != 'X':
            bool = False
            if  maze.get((X,Y))['E'] == 1 and maze.get((X,Y+1)) != 'X' and X>0 and Y+1>0:
                stack.append((X,Y+1))
                bool = True
            if  maze.get((X,Y))['W'] == 1 and maze.get((X,Y-1)) != 'X' and X>0 and Y-1>0:
                stack.append((X,Y-1))
                bool = True
            if  maze.get((X,Y))['N'] == 1 and maze.get((X-1,Y)) != 'X' and X-1>0 and Y>0:
                stack.append((X-1,Y))
                bool = True
            if  maze.get((X,Y))['S'] == 1 and maze.get((X+1,Y)) != 'X' and X+1>0 and Y>0:
                stack.append((X+1,Y))
                bool = True
            if  bool == False : 
                stack.pop()
